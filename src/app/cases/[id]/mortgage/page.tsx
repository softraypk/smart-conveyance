...
export default function MortagePage() {
    ...
    const [mortages, setMortages] = useState<Mortage[]>([]);
    const [mortageToEdit, setMortageToEdit] = useState<Mortage | null>(null);
    ...
    useEffect(() => {
        if (!caseId) return;
        fetchExistingMortage();
    }, [caseId]);

    const fetchExistingMortage = async () => {
        setLoading(true);
        try {
            const response = await api(`/cases/${caseId}`, {method: "GET"});

            const data = response.results?.data;
            const mortageParties = data?.parties?.filter((p: any) => p.role === "MORTGAGE_BROKER") || [];

            const formattedMortgages: Mortage[] = mortageParties.map((b: any) => ({
                id: b.id,
            ...
            setMortages(formattedMortgages);
        } catch (err: any) {
    ...
    const handleEdit = (buyer: Mortage) => {
        setMortageToEdit(buyer);
    };

    const handleFormSuccess = () => {
        // after create or update, reload buyer list
        fetchExistingMortage();
        setMortageToEdit(null);
    };
                    ...
                    <div className="px-4 md:px-10 lg:px-40 flex justify-center py-5">
                        <div className="flex flex-col max-w-[960px] flex-1">
                            ...
                            <div className="bg-white dark:bg-subtle-dark/50 rounded-lg p-6 shadow-sm">
                                ...
                                {/* ✅ Pass mortageToEdit and callback to form */}
                                <CaseMortgageForm
                                    caseId={caseId}
                                    mortgageToEdit={mortageToEdit}
