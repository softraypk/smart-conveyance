...
interface CaseBuyerFormProps {
    caseId: string;
    buyerToEdit?: any | null;
    onSuccess?: () => void;
}

export default function CaseSellerForm({caseId, buyerToEdit, onSuccess}: CaseBuyerFormProps) {
    ...
    useEffect(() => {
        if (buyerToEdit) {
            ...
            setEmail(buyerToEdit.user?.email || buyerToEdit.email || "");
            setPhoneNumber(buyerToEdit.phone || "");
        } else {
            setName("");
            setEmail("");
            setPhoneNumber("");
    ...
    const handleSaveForm = async (e: FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success(`Buyer ${buyerToEdit ? "updated" : "created"} successfully`);
    ...
    return (
        <form onSubmit={handleSaveForm} className="bg-white dark:bg-slate-800 rounded-lg p-8 mt-6">
            ...
            <div className="flex justify-end items-center pt-6 mt-6">
                ...
                >
                    {loading ? "Saving..." : buyerToEdit ? "Next: Overview" : "Next: Overview"}
                </button>
            </div>
        </form>
    );
